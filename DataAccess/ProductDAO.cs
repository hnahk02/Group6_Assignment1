    ...
    public class ProductDAO
    {
        ...
        public IEnumerable<Product> List()
        {
            ...
            try
            {
                ...
                {
                    products = fstoreDB.Products.Include(p => p.Category).ToList();
                }
            }
            catch (Exception e)
            {
        ...
        public IEnumerable<Product> FindAll(Expression<Func<Product, bool>> predicate)
        {
            ...
            try
            {
                using (var fstoreDB = new eStoreContext())
                {
                    products = fstoreDB.Products.Include(p => p.Category).Where(predicate).ToList();
                }
            }
            catch (Exception e)
            {
        ...
        public void Add(Product product)
        {
            try
            {
                ...
                if (p == null)
                {
                    using (var fstoreDB = new eStoreContext())
                    {
                        fstoreDB.Entry<Product>(product).State = EntityState.Added;
